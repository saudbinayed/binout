The below table helps to make sure a given database will be included in the `binout` returned by `LS-DYNA`. For example, for "nodout" to be included, the keywords: `*Database_Nodout`and `*Database_History_Node_<option>` must be in the input keyword (`.k`) file. If `Binary`is set to `2` (which is *not* the default value) in the `*Database_Nodout` keyword, and once the model is run, a `binout` file is generated by `LS-DYNA` which will include the "nodout" database. 
