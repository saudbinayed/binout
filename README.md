...
<details>
<summary>What Is a <code>binout</code>?</summary>

<!-- ### What Is a `binout`? -->
A binout is a binary file generated by [LS-DYNA](https://lsdyna.ansys.com/), now known as Ansys LS-DYNA, after running an `LS-DYNA` model, and it contains various result 
databases, such as "matsum" (for material summary, like energy and rigid body velocities' data), "nodout" (for nodal data, like displacements and velocities), "elout" (for element data, like stresses, stress-resultants, and strains), and so on. See [how to get a binout file?](#how-to-get-a-binout-file) for detail.
</details>
...
###
<details>
<summary><h4>Further Details:</h4></summary>
...
Every directory (and sub-directory) in the original `binout` becomes a field (and sub-field) in the returned `binin`. 
...
The fields of the "data" structure are the actual result (or state) data of interest. Among the fields of the "metadata" structure is a field called "ids" that stores the IDs of the model entities, e.g. nodes, parts, etc.
...
#### Note
We would like to explain choosing the word "binin": it is meant 
